...
def shear(Sh_x,Sh_y):
    ...
    print(x1,y1)
    # X-Y shearing
    x1, y1 = x1+Sh_x*y1, y1+Sh_y*x1  
    x2,y2 = x2+Sh_x*y2, y2+Sh_y*x2
    x3,y3 = x3+Sh_x*y3, y3+Sh_y*x3
    a = PutPixle(win,x1, y1)
    b = PutPixle(win,x2,y2)
    c = PutPixle(win,x2,y2)
    d = PutPixle(win,x3,y3)
...
def main():
    x = input("enter x the shearing factor:")
    y = input("enter y the shearing factor:")
    shear(int(x),int(y))
